// ==Bookmarklet Script==
// @name         OpenPhone Gemini Summarizer (Pro Config)
// @description  Adds a full settings panel for date filtering and dynamic prompt sections, with a preview modal, clipboard copy, and toast notifications.
// @version      14.0
// @author       ilakskills
// ==/Bookmarklet Script==
...
`;
    ...
    const DEFAULT_SETTINGS = {
        apiKey: '',
        dateFilterType: 'today', // 'today', 'yesterday', 'custom'
        customStartDate: new Date().toISOString().split('T')[0],
        customEndDate: new Date().toISOString().split('T')[0],
        selectedSections: Object.keys(PROMPT_SECTIONS) // Default to all sections enabled
    };
    ...
    const injectStyles = () => {
        ...
        style.innerHTML = `
            ...
            .settings-modal .gemini-modal-body { font-family: sans-serif; white-space: normal; line-height: 1.5; background-color: #fff; }
    ...
    // --- SETTINGS FUNCTIONS ---
    const getSettings = () => {
        const stored = localStorage.getItem('gemini_summarizer_settings');
        return stored ? { ...DEFAULT_SETTINGS, ...JSON.parse(stored) } : DEFAULT_SETTINGS;
    };
    const saveSettings = (settings) => {
        localStorage.setItem('gemini_summarizer_settings', JSON.stringify(settings));
    };
    ...
    const showSettingsModal = () => {
        ...
        const sectionsHtml = Object.keys(PROMPT_SECTIONS).map(key => {
            ...
            const isChecked = settings.selectedSections.includes(key);
            return `<label><input type="checkbox" name="section" value="${key}" ${isChecked ? 'checked' : ''}>${label}</label>`;
        }).join('');

        overlay.innerHTML = `
            <div class="gemini-modal-content">
                <div class="gemini-modal-header"><h2>Summarizer Settings</h2><button class="gemini-modal-close">×</button></div>
                <div class="gemini-modal-body">
                    ...
                    <div class="settings-section">
                        ...
                        <label for="startDate">Start Date</label><input type="date" id="startDate" value="${settings.customStartDate}">
                        <label for="endDate">End Date</label><input type="date" id="endDate" value="${settings.customEndDate}">
                    </div>
                    <div class="settings-section">
                ...
                <div class="gemini-modal-footer">
                    <button class="gemini-modal-button primary" id="save-settings">Save Settings</button>
                </div>
            </div>
        `;
        ...
        overlay.querySelector('#save-settings').addEventListener('click', () => {
            const newSettings = {
                apiKey: overlay.querySelector('#apiKey').value,
            ...
            // Determine dateFilterType for easier logic later
            const today = new Date().toISOString().split('T')[0];
            const yesterday = new Date(Date.now() - 86400000).toISOString().split('T')[0];
            if (newSettings.customStartDate === today && newSettings.customEndDate === today) {
                newSettings.dateFilterType = 'today';
            } else if (newSettings.customStartDate === yesterday && newSettings.customEndDate === yesterday) {
                newSettings.dateFilterType = 'yesterday';
            } else {
                newSettings.dateFilterType = 'custom';
            }
    ...
    const filterActivitiesByDate = (activities, settings) => {
        let startDate, endDate;
        const now = new Date();
        const todayStart = new Date(now.getFullYear(), now.getMonth(), now.getDate());
        const yesterdayStart = new Date(now.getFullYear(), now.getMonth(), now.getDate() - 1);
        
        switch (settings.dateFilterType) {
            case 'today':
                startDate = todayStart;
                endDate = new Date(todayStart.getTime() + 86400000); // End of today
                break;
            case 'yesterday':
                startDate = yesterdayStart;
                endDate = new Date(yesterdayStart.getTime() + 86400000); // End of yesterday
                break;
            case 'custom':
    ...
    const buildDynamicPrompt = (settings) => {
        let structuredFormat = '';
        settings.selectedSections.forEach(key => {
    ...
    const runSummaryProcess = async (buttonElement) => {
        ...
        const settings = getSettings();
        if (!settings.apiKey) {
            showToast("Gemini API Key not set. Please set it in Settings (gear icon).", 'error');
        ...
        try {
            ...
            const openPhoneApiUrl = `https://communication.openphoneapi.com/v2/activity?id=${conversationId}&last=200`; // Fetch more activities for filtering
            ...
            // Apply filtering and build dynamic prompt
            ...
            const geminiApiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key=${settings.apiKey}`;
