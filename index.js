...
// @require      https://cdnjs.cloudflare.com/ajax/libs/jszip/3.10.1/jszip.min.
// @grant        GM_addStyle
// @grant        GM_setValue
// @grant        GM_getValue
...
(function() {
    ...
    /**
     * A wrapper for GM_xmlhttpRequest that mimics the Fetch API's Promise-based interface.
     * @param {string} url The URL to request.
     * @param {object} options Options similar to fetch(), including method, headers, body, and responseType.
     * @returns {Promise<object>} A promise that resolves with an object mimicking a fetch Response.
     */
    function gmFetch(url, options = {}) {
        return new Promise((resolve, reject) => {
            GM_xmlhttpRequest({
                ...
                data: options.body,
                responseType: options.responseType, // e.g., 'blob', 'json'
                onload: (response) => {
                    const mockResponse = {
                        ...
                        // GM_xmlhttpRequest provides the direct response object for blob/json types
                        response: response.response,
                        responseText: response.responseText,
                        // Mimic Fetch API methods
    ...
    const downloadSummaryAsZip = async (summaryText, filename, imageInfo) => {
        try {
            ...
            // JSZip is loaded via @require, so window.JSZip is available
            ...
            if (imageInfo && imageInfo.length > 0) {
                ...
                const imageFetchPromises = imageInfo.map(async (info) => {
                    try {
                        // Use gmFetch to get the image as a blob
                        const response = await gmFetch(info.url, { responseType: 'blob' });
                        if (!response.ok) throw new Error(`Failed to fetch image: ${info.url}`);
                        const blob = response.response; // The blob is in the .response property
            ...
            link.download = filename.replace(".txt", ".zip");
            link.click();
        ...
        try {
            ...
            if (imageMediaItems.length > 0) {
                ...
                const urlToDataUrl = async (url) => {
                    try {
                        const response = await gmFetch(url, { responseType: 'blob' });
